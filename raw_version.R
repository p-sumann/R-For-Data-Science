...
freq(SAQ8$q01, cumul = TRUE, round.digits = 1, style = st_options("style"))
freq(SAQ8$q03, cumul = TRUE, round.digits = 1)
freq(SAQ8$q06, cumul = TRUE, round.digits = 1)
freq(SAQ8$q08, cumul = TRUE, round.digits=1)
...
library(rvest)
library(netstat)
rD <- rsDriver(browser="firefox",verbose = F, port = free_port())
remDr <- rD[["client"]]
